...
function AuthForm({register, afterSubmit}: Props) {
  ...
  return (
    <>
      {visible ? (
        <RegisterForm onToggleForm={toggle} onFinish={registerAccount} onFinishFailed={onFinishFailed} />
      ) : (
        <LoginForm onToggleForm={toggle} onFinish={loginAccount} onFinishFailed={onFinishFailed} />
      )}
    </>
  );
}
